...
function collectionCard(product) {
  ...
  return `
    <div class="product-card group relative rounded-lg overflow-hidden bg-white/40">
      ...
      <button class="wishlist-btn ${wishlisted ? "is-active" : ""} absolute bottom-[52px] sm:bottom-[68px] right-2 bg-white/90 rounded-full w-7 h-7 flex items-center justify-center" data-product-id="${product.id}" aria-label="Toggle wishlist">
...
function getActiveFilters() {
  const active = { occasion: [], color: [], section: [] };
...
function applyFilters() {
  ...
  const results = PRODUCTS.filter((p) => {
    ...
    const matchesSection = filters.section.length === 0 || filters.section.includes(p.section);
    const matchesSearch = !query || p.name.toLowerCase().includes(query);
    return matchesOccasion && matchesColor && matchesSection && matchesSearch;
  });
